...
const shell = initShell({
  app: { id: 'demo', name: 'Shell Demo', icon: icon('gauge', 22) },
  sections: SECTIONS,
  onNav: show,
  isAdmin: () => true,
  uiMode: () => getUiMode(),
...
// ---- site-chrome: the shared public-site header + footer (docs/BRAND.md) ------
// Not part of the in-app shell above — this is the marketing-page chrome. Mount
// a sample of each so the smoke gate exercises both builders (icons resolve,
// DOM renders, zero pageerrors). Kept out of the app-shell viewport flow.
const chromeWrap = el('div', { 'data-demo': 'site-chrome', 'aria-hidden': 'true',
  style: 'position:absolute;left:-9999px;top:0;width:1080px' });
chromeWrap.append(
  el('header', { id: 'demoSiteHeader' }),
  el('footer', { id: 'demoSiteFooter' }),
  el('footer', { id: 'demoRootFooter' }),
);
document.body.append(chromeWrap);
siteHeader('#demoSiteHeader', {
  app: 'manager',
  nav: [{ href: '#features', label: 'Features' }, { href: '#how', label: 'How it works' }],
  cta: { href: '/app/', label: 'Launch app' },
});
siteFooter('#demoSiteFooter', { app: 'manager', docs: '/app/#docs', appUrl: '/app/', notices: 'THIRD-PARTY-NOTICES.md' });
siteFooter('#demoRootFooter', { root: true,
  links: [{ href: '#apps', label: 'Apps' }, { href: '#work', label: 'Consulting' }],
  meta: '© 2026 Polecat.live · GPL-3.0' });
