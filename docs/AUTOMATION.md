...
| `steward-focus.yml` | heartbeat tick every 10 min (`*/10`, Claude-free) | **The multi-app focus roster.** Reads `.github/steward/focus.json` through `.github/steward/schedule.mjs` (the canonical evaluator) and, each tick, dispatches one focus improve run per enabled lane that is due AND idle — so a lane advances to its next unit within ~10 min of the last finishing, and a broken chain restarts on the next tick rather than stalling an hour. Drop the cron to `*/5` to run harder. Lane schedule fields: `enabled`, `everyHours`, `offset` (align which hours the cadence lands on), `window` (UTC hour window, wraps midnight), `startAt` (sleep until), `until` (expire at — "run every X until Y"), `slices` (1..5, default 1 — how many SEQUENTIAL improve runs the lane produces each fire; each is a full unit of work with its own PR + smoke gate). Slices are **chained, not fired at once**: steward-focus dispatches one slice-1 run carrying the count, and each `steward-improve` run dispatches the next on completion (its "Chain next slice" step) — firing them all up front would overflow the per-app concurrency queue (one running + one pending) and GitHub would cancel the surplus. A chain that outruns the hour just continues: the next tick skips an app that still has a run in flight, so no competing chain is injected. Different apps run in parallel; the same app never overlaps itself. Edit lanes from Manager's Fleet Ops panel, the GitHub UI, or any session — effective next tick, no workflow edits. Preview with `node .github/steward/schedule.mjs next`. |
