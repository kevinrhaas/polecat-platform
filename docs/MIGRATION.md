...
| 6 | **analytics** | L | **ASSIGNED to a dedicated session (2026-07-15) — scheduled steward runs SKIP this app** (and its open steward/* PRs). 22.6K LOC, own shell, strict STATUS.md conventions. The dedicated session adopts the platform PROCESS immediately (PR flow, stamping, no push to main) and stages the UI shell adoption carefully — chrome-level pieces (tokens/theme/waffle) first; the Studio three-pane workspace waits for shell v2 unless the session upgrades the shell itself via platform PRs. Do not disturb the studio's spec/export invariants. |
